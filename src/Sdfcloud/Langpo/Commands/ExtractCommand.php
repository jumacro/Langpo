...
class ExtractCommand extends Command {
     ...
     * 
     * @return void
     * @throws \Sdfcloud\Langpo\XgettextException
     */
    public function fire() {
        ...
        if ($i < 1) {
            // throw exception
            //throw new \Sdfcloud\Langpo\NoFilesToExtractFromException("No templates found");
            ...
            if ($this->confirm("  would you like to run the compiler using the default settings? [y|n]")) {
                ...
                $this->call('langpo:compile'); // call compile command
                // re-check
         ...
         */
        $xgettext_output_file = storage_path() . DIRECTORY_SEPARATOR . $this->option("output_folder") . DIRECTORY_SEPARATOR . Config::get("langpo::config.textdomain") . ".pot";
        ...
        if (!$process->isSuccessful()) {
            throw new \Sdfcloud\Langpo\XgettextException("The xgettext command could not be executed:" . PHP_EOL .
    ...
    protected function getOptions() {
         ...
         * set defaults
         */
        $binary = Config::get("langpo::config.xgettext.binary");

        $defaults = array(
            'binary' => (isset($binary) == true) ? $binary : "xgettext",
            'binary_path' => Config::get("langpo::config.xgettext.binary_path"),
            'language' => Config::get("langpo::config.xgettext.language"),
            'comments' => Config::get("langpo::config.xgettext.comments"),
            'force_po' => Config::get("langpo::config.xgettext.force_po"),
            'input_folder' => Config::get("langpo::config.xgettext.input_folder"),
            'views_folder' => Config::get("langpo::config.compiler.input_folder"),
            'output_folder' => Config::get("langpo::config.xgettext.output_folder"),
            'from_code' => Config::get("langpo::config.xgettext.from_code"),
            'copyright_holder' => Config::get("langpo::config.xgettext.copyright_holder"),
            'package_name' => Config::get("langpo::config.xgettext.package_name"),
            'package_version' => Config::get("langpo::config.xgettext.package_version"),
            'email_address' => Config::get("langpo::config.xgettext.email_address"),
            'keywords' => Config::get("langpo::config.xgettext.keywords"),
